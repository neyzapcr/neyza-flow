...
export default function BeasiswaForm() {
  ...
  function validate(field, value) {
    ...
    if (field === "namaLengkap") {
      if (!value.trim()) newErrors.namaLengkap = "Nama tidak boleh kosong.";
      else if (/\d/.test(value)) newErrors.namaLengkap = "Nama tidak boleh mengandung angka.";
      else delete newErrors.namaLengkap;
    }
    ...
    if (field === "email") {
      if (!value.trim()) newErrors.email = "Email tidak boleh kosong.";
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) newErrors.email = "Format email tidak valid.";
      else delete newErrors.email;
    }

    if (field === "jurusan") {
      if (!value) newErrors.jurusan = "Pilih jurusan.";
      else delete newErrors.jurusan;
    }

    if (field === "semester") {
      if (!value) newErrors.semester = "Pilih semester.";
      else delete newErrors.semester;
    }
    ...
    <Container>
      <form onSubmit={handleSubmit}>
        <div className="grid gap-2 md:grid-cols-2 ">
          ...
          <InputField label="Email Aktif" type="email" placeholder="nama@gmail.com" value={email}
