  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Normalize segment column to capitalize (e.g. "VIP", "Loyal", "Regular", "New")
        const mapped = (data || []).map(c => {
          let seg = c.segment || "New";
          if (seg.toLowerCase() === "vip") seg = "VIP";
          else if (seg.toLowerCase() === "loyal") seg = "Loyal";
          else if (seg.toLowerCase() === "regular") seg = "Regular";
          else if (seg.toLowerCase() === "new") seg = "New";
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6 text-left">
        ...
        <Card>
          ...
          <p className="text-xs text-gray-400 mb-3 bg-white">Total {customers.length} pelanggan</p>
          ...
          <div className="space-y-2">
            {segmentData.map((s, i) => (
              <div key={s.name} className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-gray-700 bg-white">{s.value} pelanggan</span>
              </div>
            ))}
          </div>
        </Card>
        ...
        <div className="lg:col-span-2 grid grid-cols-2 gap-3">
          {Object.entries(segmentConfig).map(([seg, cfg]) => {
            ...
            const avgSpent = Math.round(list.reduce((s, c) => s + Number(c.totalSpent || 0), 0) / (count || 1));
            return (
              <div
                key={seg}
              ...
              >
                <div className="flex items-center justify-between mb-2 bg-white">
                  ...
                  <span className="text-2xl font-bold text-gray-800 bg-white">{count}</span>
                </div>
                <p className="text-xs text-gray-500 leading-relaxed bg-white">{cfg.desc}</p>
                <p className="text-xs text-gray-400 mt-2 bg-white">Rata-rata belanja: <span className="font-semibold text-gray-700">Rp {avgSpent.toLocaleString("id-ID")}</span></p>
              </div>
            );
          })}
        </div>
        ...
        <Table headers={["Pelanggan", "Segmen", "Frekuensi", "Total Belanja", "Transaksi Terakhir", "Status"]}>
          {paginatedFiltered.map((c) => (
            <tr key={c.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-5 py-4 font-semibold text-gray-700">{c.totalTransactions || 0}x</td>
              <td className="px-5 py-4 font-bold text-gray-800">Rp {(c.totalSpent || 0).toLocaleString("id-ID")}</td>
